...
public class Ghost extends Sprite {
    ...
    public Ghost(World world, PlayScreen screen,int x) {
        int yy=0;
        enemyattacking= new Texture("ghost-shriek.png");
        enemystanding= new Texture("ghost-idle.png");

        enemydying =new Texture("ghost-vanish.png");
        ...
        for (int i = 0; i < 6; i++)
            frames.add(new TextureRegion(enemystanding, i*448/7, 0, 448/7, 80));
        enemyStand = new Animation(0.1f, frames);

        frames.clear();

        for (int i = 0; i < 8; i++)
            frames.add(new TextureRegion(enemydying, i*448/7, 0, 448/7,80));
...
//            attack = true;
    ...
    public void update(float dt) {
        if(b2body.getPosition().y<90 && !Hurt)
        b2body.applyLinearImpulse(new Vector2(0,10000),b2body.getWorldCenter(),true);
